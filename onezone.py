# onezone.py
# One zone model for Type I X-ray bursts

from scipy.integrate import ode
from scipy import optimize
import numpy
import matplotlib.pyplot as plt
import sys
from math import exp

def pressure(rho,T,Ye,Yi):
	Ped=9.91e12*(Ye*rho)**(5.0/3.0)   # non-relativistic degenerate electron pressure
	Pend=Ye*rho*1.38e-16*T/1.67e-24   # non-degenerate electron pressure 
	Pe=(Ped**2+Pend**2)**0.5   # fitting formula for electron pressure (Pacynski 1983)
	Pion=Yi*rho*1.38e-16*T/1.67e-24	# ion pressure
	Prad=arad*T**4/3.0	# radiation pressure
	return Pion+Pe+Prad

def find_rho_eqn(rho,P,Ye,Yi,T):
	return pressure(rho,T,Ye,Yi)-P

def find_rho(P,T,Ye,Yi):
	# inverts the equation of state to find the density
	rho = optimize.brentq(find_rho_eqn,1.0,1e8,xtol=1e-6,args=(P,Ye,Yi,T))
	return rho
	
def kappa(rho5,T8,Ye):
	# radiative opacity
	kff = 0.4*0.753*Ye*rho5/T8**3.5    # free-free Gaunt factor set to 0.4
	kes = 0.4*Ye/((1+2.7*rho5/T8**2)*(1.0+(T8/4.5)**0.86))
	# Thompson scattering with corrections from Paczynski 1983
	return kes + kff   # note that strictly Rosseland mean opacities don't add (<30% error)
	
def derivs(t,z):
	T = z[0]
	T8=1e-8*T
	ycolumn = z[1]
	rho5 = 1e-5 * find_rho(gravity*ycolumn,T,Ye,Yi)

	# nuclear burning rate for triple alpha
	eps3a=5.3e21*rho5**2*XHe**3*exp(-44.0/T8)/T8**3
	# boost the triple alpha energy release to burn to iron group
	eps3a*=(1.6+(1.0-XHe)*4.9)/0.606
	# and include hot CNO burning and the base flux
	eps = eps3a + 5.8e15*0.01 + Qbase*mdot/ycolumn

	# assume ideal gas for heat capacity of ions and electrons
	cp = 2.5*(Yi+Ye)*kB/mproton
	cp += 4.0*arad*1e19*T8**3/rho5;
    
	# cooling rate
	eps_cool = clight * arad * T**4 / (3.0 * kappa(rho5,T8,Ye) * ycolumn**2)

	# calculate derivatives
	dTdt = (eps - eps_cool) / cp
	dydt = mdot - 12.0 * eps3a * ycolumn/Q3a
	
	return [dTdt,dydt]
	

# parameters
arad=7.5657e-15
clight=3e10
gravity = 1.9e14
mdot_Edd = 8.8e4
mproton = 1.67e-24
kB = 1.38e-16
Q3a = 7.275
# adjust fluxes into per gram units
Qconv = 1.6e-6/mproton   # 1 MeV per nucleon in erg/g
Q3a*=Qconv

# accretion rate from the command line
if (len(sys.argv)<3):
	print 'Syntax: onezone.py <mdot> <time>'
	exit()
mdot=float(sys.argv[1])
mdot *= mdot_Edd
tend=float(sys.argv[2])  # total time to run
Qbase = 0.15*Qconv   # heating from below in MeV/nucleon
# He mass fraction
XHe=0.3
Ye=1.0-XHe/2.0
Yi=1.0-3.0*XHe/4.0

# Integrate
solver=ode(derivs)
t=[]
result=[]
inic = [2e8,2e8]  # initial temperature and column depth
solver.set_initial_value(inic,0.0)
solver.set_integrator('vode')
while solver.successful() and solver.t<tend:
	solver.integrate(tend,step=True)
	t.append(solver.t)
	result.append(solver.y)
t=numpy.array(t)
result=numpy.array(result)

# PLOT THE RESULTS

# Plot the temperature against time
#plt.plot(t,result[:,0])
#plt.show()

# Calculate the light curve
lum = numpy.array([])
llast = -10.0
tlast=0.0
goingup=True
deltat=0.0
for counter, T in enumerate(result[:,0]):
	ycolumn = result[counter,1]
	
	rho5 = 1e-5 * find_rho(gravity*ycolumn,T,Ye,Yi)
	T8=1e-8*T

	eps_cool = clight * arad * T**4 / (3.0 * kappa(rho5,T8,Ye) * ycolumn**2)
	luminosity = 4.0*3.1415*1e12 * ycolumn * eps_cool
	if (luminosity > llast):
		goingup=True
	else:
		if (goingup==True):
			deltat = t[counter]-tlast
			tlast=t[counter]
		goingup=False
	llast=luminosity
	lum=numpy.append(lum, luminosity)

print 'Time between flashes=',deltat
n=len(lum)
print 'Luminosity range = ',max(lum[0.5*n:])-min(lum[0.5*n:])

plt.plot(t,lum)
plt.xlabel('Time (s)')
plt.ylabel('Luminosity (erg/s)')
plt.show()
#plt.savefig('lc.pdf')

# Plot the temperature against column
plt.plot(result[:,1],result[:,0])
plt.xlabel('Column depth (g/cm^2)')
plt.ylabel('Temperature (K)')
plt.show()
#plt.savefig('yT.pdf')


